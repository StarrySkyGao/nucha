...
// 定义一个名为forward_mlp的函数，参数为两个GEMM_W4A4类型的变量fc1和fc2，以及一个Tensor类型的变量norm_hidden_states
Tensor forward_mlp(GEMM_W4A4 &fc1, GEMM_W4A4 &fc2, Tensor norm_hidden_states) {
    // 调用fc1的forward函数，传入norm_hidden_states和GEMM_W4A4::FuseOptions::GELU_QUANT，并将结果传入fc2的forward_quant函数
...
// 构造函数，初始化FluxSingleTransformerBlock类
...
Tensor FluxSingleTransformerBlock::forward(Tensor hidden_states, Tensor temb, Tensor rotary_emb) {
    ...
    if (attnImpl == AttentionImpl::FlashAttention2) {
        ...
        // 调用attention_fp16函数进行注意力计算
        kernels::attention_fp16(q, k, v, o, pow(dim_head, (-0.5)));

        // 根据batch_size和num_tokens_pad的值选择不同的操作
...
// 构造函数，初始化JointTransformerBlock类
